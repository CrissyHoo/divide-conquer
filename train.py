...
from models.network_flownetsd import FlowNet2SD as flownet
import json
...
def train(rank, config):
    ...
    # flowmodel = flownet().to(device)
    # resume = "./model_zoo/FlowNet2-SD_checkpoint.pth.tar"
    # checkpoint = torch.load(resume)['state_dict']
    # flowmodel.load_state_dict(checkpoint)
...
def test(rank, config):
    ...
    # singlemodel = config.network2
    # singlemodel = singlemodel.to(device)
    # # singlemodelpath = os.path.join("/project/husicheng/data/checkpoint/lovsr_8+4_80frame/0101.pth")
    # singlemodelpath = os.path.join("/project/husicheng/currivulumloss_tmp-main/model_zoo/0316.pth")
    # singlemodel=DDP(singlemodel, device_ids=[rank], output_device=rank)
    # # checkpoint = torch.load(singlemodelpath, map_location=lambda storage, loc: storage.cuda(rank))
    # # singlemodel.load_state_dict(checkpoint["model"])#
    # checkpoint = torch.load(singlemodelpath, map_location=lambda storage, loc: storage.cuda(rank))
    # singlemodel.load_state_dict(checkpoint["model"])
