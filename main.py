...
if __name__=='__main__':
    ...
    config.network = importlib.import_module(f'models.{config.model.file}').Net(config)#.Net(config)
    config.network2 = importlib.import_module(f'models.{config.model.file}').singleNet(config)
    # config.network2 = importlib.import_module(f'models.rrdbnet').RRDBNet(3, 3, 64, 23, gc=32)
