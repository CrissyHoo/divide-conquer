...
def single_load(hr, mode='train', crop_size=32,scale=4, num_frame=7):
    ...
    for i in range(4):
        #用4帧来补偿一下,无论是train还是不train，都使用64的size去随机crop来补偿
        tmp=single_crop(hr_img,size=crop_size)
        patch_pool.append(tmp)

    return hr_img,patch_pool
...
class loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_kind == 'double':
            ...
            HR,patchpool = single_load(self.hqfiles[index], mode=self.mode, crop_size=self.crop_size, scale=self.scale, num_frame=self.num_frame)

        HR = torch.from_numpy(HR / 255.).float().permute(3,0,1,2)
        patch_pool = []
        for patch in patchpool:
            patch_pool.append(torch.from_numpy(patch / 255.).float().permute(0, 3, 1, 2))

        patch_pool = torch.stack(patch_pool)  # 4chw

        if self.data_kind == 'double':
            return LR, HR
        else:

            return HR, self.priors[index], index, patch_pool
